...
| 项 | 内容 |
|----|------|
| **目标** | 用 geopandas 读矢量并打印元信息 |
| **条件** | `data/raw/` 下有 `.shp`（含 .prj 等）或 `.geojson` |
| **任务** | `gpd.read_file`；打印 CRS、`len(gdf)`、字段名、`gdf.head(3)` |
| **要求** | 能跑通；CRS 能说出名字或 EPSG 号 |
| **结果** | `src/week1/d02_read_vector.py` + 终端输出截图 |
| **验收** | 控制台四项信息齐全 |
| **程度** | 能跑通；坐标系说得出名字即可 |
